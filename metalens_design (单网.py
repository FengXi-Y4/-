...
def generate_target_phase(diameter: float, focal_length: float, 
    ...
    try:
        ...
        for λ in wavelengths:
            # 理想透镜相位分布
            #k = 2 * np.pi / λ                                                  #相位值，后面需要再减去起点值
            k = 2 * np.pi * (  1 / λ  -  1/650  )                               #相位补偿值，后面的不需要再减phase[-1]
            phase = - k * (np.sqrt(r**2 + focal_length_nm**2) - focal_length_nm)  #这里有没有+ -？？？
    ...
    try:
        ...
        for key in required_keys:
            if key not in params:
                raise ValueError(f"归一化参数缺少关键字段: {key}")
        
        logging.info(f"归一化参数已加载: {norm_path}")
        logging.info(f"  相位范围: [{params['Z_min']:.2f}, {params['Z_max']:.2f}] rad")
...
def generate_metalens_design(model: MetaLensNet, target_phase: np.ndarray, 
    ...
    try:
        ...
        with tqdm(total=valid_pixels, desc="生成纳米柱设计") as pbar:
            for i in range(resolution):
                for j in range(resolution):
                    ...
                    # 与训练预处理完全对齐 - 减去初始相位（phase_profile[0]对应650nm波长，与训练顺序完全一致）
                    phase_profile = phase_profile - phase_profile[0]
                    
                    # WAVELENGTHS已直接设置为1002个点，无需插值，直接使用phase_profile
                    phase_interp = phase_profile
                    
                    # 使用训练时的GLOBAL全局归一化
                    phase_norm = (phase_interp - Z_MIN) / (Z_MAX - Z_MIN + 1e-8)
                    
                    # ========== 步骤2: 将相位值导入神经网络获取纳米柱尺寸 ==========
                    ...
                    # ========== 步骤3: 计算偏转角2 ==========
                    s = np.sqrt(x**2 + y**2)
                    theta2_rad = 1 * np.pi * (focal_length - np.sqrt(s**2 + focal_length**2)) / lambda_um   #PB相位
                    theta2_deg = theta2_rad * 180.0 / np.pi   #这里已经是偏转角了，不需要/2了
        ...
        # ===== 验证相位偏移范围 =====
        valid_mask = ~np.isnan(phase_shift_2_rad_map)
        ...
        with open(txt_path, 'w') as f:
            ...
            f.write("# 格式: x坐标(μm)  y坐标(μm)  长度L(nm)  宽度W(nm)  偏转角2(rad)  偏转角2(deg)\n")
            ...
            for i in range(resolution):
                for j in range(resolution):
                    ...
                    f.write(f"{x:>10.4f} {y:>10.4f} {L:>10.2f} {W:>10.2f} {p2_rad:>14.6f} {p2_deg:>12.4f}\n")
                    nanopillar_count += 1
        ...
        if nanopillar_count > 0:
            ...
            logging.info(f"  格式: x y L W theta2_rad theta2_deg")
        else:
...
def generate_design_visualizations(L_map: np.ndarray, W_map: np.ndarray, output_dir: str):
    ...
    plt.title('圆形超透镜纳米柱长度分布')
    plt.xlabel('像素')
    plt.ylabel('像素')
    plt.savefig(os.path.join(output_dir, 'L_map_visualization.png'), dpi=150)
    plt.close()
    ...
    plt.title('圆形超透镜纳米柱宽度分布')
    plt.xlabel('像素')
    plt.ylabel('像素')
    plt.savefig(os.path.join(output_dir, 'W_map_visualization.png'), dpi=150)
    plt.close()
    ...
    plt.title(f'长度 L 分布 (圆形区域, N={len(L_valid)})')
    plt.xlabel('长度 (nm)')
    plt.ylabel('频率')
    ...
    plt.title(f'宽度 W 分布 (圆形区域, N={len(W_valid)})')
    plt.xlabel('宽度 (nm)')
    plt.ylabel('频率')
    plt.grid(True, alpha=0.3)
...
def create_design_report(L_map: np.ndarray, W_map: np.ndarray,
    ...
    with open(report_path, 'w') as f:
        ...
        f.write("                        格式: x(μm) y(μm) L(nm) W(nm) theta2(rad) theta2(deg)\n\n")
...
def main():
    ...
    FOCAL_LENGTH = 80.0  # 焦距 (μm)
    WAVELENGTHS = np.linspace(650, 450, 1002).tolist()  # 工作波长范围 (nm)，与训练时顺序一致：从650→450（频率从小到大），共1002个点
    ...
    input_dim = 1002  # 与训练模型保持一致: 1000频率点插值 + 2目标维度
    ...
    L_map, W_map = generate_metalens_design(model, target_phase, WAVELENGTHS, 
                                           NANOCELL_PERIOD, FOCAL_LENGTH, norm_params, OUTPUT_DIR)
    
    # 4. 创建设计报告
    logging.info("创建设计报告...")
    create_design_report(L_map, W_map, WAVELENGTHS, DIAMETER, FOCAL_LENGTH, OUTPUT_DIR)
